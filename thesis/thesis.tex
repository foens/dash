...
When \DASH begins execution, the region graph is equivalent to the control flow graph of the program and all regions contain the single predicate \code{true}, which allows any variable to take any value. Whenever the \DASH algorithm refines the abstraction, a region is split into two using a predicate that is suitable for refinement. The main algorithm, including refinement, will be explained in following sections. 
...
\def \height {6cm}
	...
	\caption
	{
		...
		\protect\subref{fig:dashInt:refinegraph:aftersplitnewtrace} shows the a new trace found in the next iteration. The frontier edge has been pushed backwards.
	}
...
Initialization of \classname{ExecuteSymbolic} is completed by the lines 1-3 in \cref{alg:dashInt:executeSymbolic}. The region predicate $\rho_0$ is unpacked from region $S_0$ in line 1. Initial symbolic variable $v_0$ are assigned to each of the input variables $v \in \classname{params}(P)$ in line 2. Line 3 assigns the symbolically evaluated region predicate of $S_0$ to the path constraint $\phi$. Notice that in \DASHint, the predicate attached to region $S_0$ is always \code{true} due to how \classname{RefineGraph} handles refinement of the initial region described in \cref{sec:dashInt:refinegraph:initialRegion}. However, in \DASHcall initial regions can have non-\code{true} predicates assigned to them, and we therefore symbolically executes them in \DASHint as well.
...
We also slightly altered what a trace is. The \DASH article has an ordered path $\tau_o = S_0, \ldots, S_{k-1}, S_k, \ldots S_n$ which is a concatenation of regions that follow a concrete execution $s_0, \ldots, s_{k-1}$ and the rest of the abstract error path $S_k, \ldots, S_n$. However, only the frontier edge from the abstract error path, located at $(S_{k-1}, S_k)$, is used by \DASH. We found it confusing to create a trace where the last part $S_{k+1}, \ldots, S_n$ is never used, and as such we have chosen to only include the single edge that becomes the frontier. This has simplified the pseudocode in that we do not need to call an auxiliary \classname{Frontier} procedure to determine where the frontier edge is located in $\tau_o$. The frontier edge in our trace $\tau_c$ is always the last edge.
